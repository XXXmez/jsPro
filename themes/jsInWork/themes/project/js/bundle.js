  ...
  function getDinamicInfo(parentSelector) {
    ...
    input.addEventListener('input', () => {
      if (input.value.match(/\D/g)) {
        // в данном случае можно еще так (isNaN(+input.value))
...
function cards() {
  ...
  class AddMenuContent {
    constructor(img, imgAlt, title, text, price, perentSelector) {
      // в rest нельзя вложить дефолтные значения
      this.img = img;
      this.imgAlt = imgAlt;
      this.title = title;
      this.text = text;
...
function forms(formSelector, modalTimerId) {
  ...
  function bindPostData(form) {
    ...
    form.addEventListener('submit', e => {
      ...
      const json = JSON.stringify(Object.fromEntries(formData.entries())); // пример использования Object.entries - возврашает преобразованный объект в массив
  ...
  fetch('http://localhost:3000/menu').then(data => data.json()).then(result => console.log());
}
...
const postData = async (url, data) => {
  // отвечает за постинг данных (отправку на сервер) / async - предупреждаем что тут будет асинхрон.
  const result = await fetch(url, {
    // await - нужно ждать загрузки этого кода
    method: "POST",
    headers: {
      'Content-type': 'application/json'
    },
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  (0,_modules_timer__WEBPACK_IMPORTED_MODULE_7__["default"])('.timer', '2022-06-01'); // tabs
  // timer
  // modal
  // cards
  // forms
